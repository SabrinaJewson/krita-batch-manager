...
class Any:
	...
	def object(self) -> "Object":
		if not isinstance(self.value, dict): self.error("expected object")
		return Object(self.value, path=self.path)
	def list(self) -> "List":
		if not isinstance(self.value, list): self.error("expected list")
		return List(self.value, path=self.path)
	def str(self) -> "Str":
		if not isinstance(self.value, str): self.error("expected str")
		return Str(self.value, path=self.path)
	def int(self) -> "Int":
		if not isinstance(self.value, int): self.error("expected integer")
		return Int(self.value, path=self.path)
	def float(self) -> "Float":
		if not isinstance(self.value, float): self.error("expected floating-point number")
		return Float(self.value, path=self.path)
	def bool(self) -> bool:
		if not isinstance(self.value, bool): self.error("expected boolean")
		return self.value
	def null(self) -> None:
		if self.value is not None: self.error("expected null")
...
class Object(Any):
	...
	def get(self, key: str) -> Any:
		if key not in self.value: self.error(f"expected key {key}")
...
class List(Any):
	...
	def get(self, i: int) -> Any:
		if i < 0: raise Exception("attempted to index with negative integer")
		if i >= len(self.value): self.error(f"expected at least {i + 1} element(s)")
		return Any(self.value[i], path=[*self.path, i])

class Str(Any):
	value: str
	def nonempty(self) -> str:
		if len(self.value) == 0: self.error(f"expected nonempty string")
		return self.value
...
class Tests(unittest.TestCase):
	...
	def test_root(self):
		self.assertEqual(self.error("[]", lambda c: c.object()), "error at root: expected object")
		self.assertEqual(self.error("5", lambda c: c.object()), "error at root: expected object")
		self.assertEqual(self.error("5.1", lambda c: c.int()), "error at root: expected integer")

	def test_nested(self):
		self.assertEqual(self.error("[2, 3]", lambda c: c.list().get(0).float()), "error at [0]: expected floating-point number")
		self.assertEqual(self.error('{"a":true}', lambda c: c.object().get("a").null()), "error at .a: expected null")

	def test_deny_unknown(self):
		self.assertEqual(self.error('{"a":true}', lambda c: c.object().deny_unknown()), "error at root: unexpected key a")
