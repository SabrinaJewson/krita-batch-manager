...
class DockWidget(krita.DockWidget):
	...
	def reload(self, kr: Krita) -> None:
		...
		WidgetFactory = Callable[[QWidget, Krita, Callable[[], None]], Widget]
		self.w = cast(WidgetFactory, g["Widget"])(self, kr, lambda: self.reload(kr))
		self.setWidget(self.w)
...
if (kr := Krita.instance()) is not None and "reloading" not in globals():
